...
logging.basicConfig(filename='warningSST.log', level=logging.WARNING)
...
#All
n_cells = 1556    # PV: 888 SST: 1823
halfcook_path='./All_figures/Cell'
filename = f'./testoutput/{date_str}_V1_Other_addon.csv'
...
# for i_cell in i_cell_vec:
    ...
    try:
        ...
        df_sweeps=df_sweeps.sort_values('Inj')
        ...
        rheo1st= Inj_vec[Ind[0]] 
        df_cells.at[i_cell,'rheo1st']=rheo1st
        
        Ind2AP=np.where( (Inj_vec>0) & (rate_vec>1) & (rate_vec<=50))[0]
        ...
        if len(Ind2AP)==0:
            ...
            Inj_vec_more= np.array([-40,-30,-20,-10])+Inj_vec[Ind[0]]
        ...
        for i_sweep in range(maxRate_ind+1):
            if df_sweeps.at[i_sweep,'nAP']> 9: #Used to be at least 5AP, now change to 10AP for fitting 6 parameters.
        ...
        if max_nAP>=10:
            ...
            legit_sweeps= (Inj_vec>rheo1st*1.5) & ~np.isnan(dAdap_vec) &  (LastSpike_vec>0.8)  
            ...
            df_cells.at[i_cell,'pvalue_ob']=df_sweeps.at[orig_pvalue_ind,'pvalue_ob']
...
'''
filename = f'./testoutput/{date_str}_till{i_cell}V1_addon.csv'
'''
