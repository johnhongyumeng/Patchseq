...
def fit_exp_nonlinear(t, y,p0=[-5, 0.01, 25]):
    opt_parms, parm_cov = sp.optimize.curve_fit(model_func, t, y, p0, maxfev=10000) # 10000 has error on 486 - try 50000
    A, K, C = opt_parms
    return A, K, C
...
#Output_df=pd.read_csv("Output_df_pv.csv")
...
#for i_cell in [5,6]:
for i_cell in range(1200, n_cells):  # a test for the first 600 # then 600-1200 # then the rest
    print(i_cell)
    cell_flag=True
    
    TempPath='SST_all_layer/Cell'+str(i_cell)+'/'
    ...
    #        plt.plot(t_relax, fit_relax, '-',linewidth=1) 
        ...
        if len(t_spike)>0 :
            ...
            # take a try, 0310
            sweep_ext = EphysSweepFeatureExtractor(t=sweep.t, v=sweep.v, i=sweep.i, start=t_start+0.0, end=sweep.t[-1])
    ...
    # print(Output_df.iloc[i_cell])


# Contains the output AI, rABC rheo Slope maxInj maxRate Flag_select
Output_df.to_csv('Output_df_sst_0315_1200_end.csv', index=False)
